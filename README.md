...
- The player dashboard presents fixtures against the display team via `fixtureCategory`: **My Team** (display team, never labelled a play-up), **Play-Up Opportunities** (the two teams immediately above the player's Registered Team, skipping teams they are already selected for, closest first - every fixture must pass the eligibility engine) and **Support Fixtures** (lower-ranked, eligibility-gated, collapsed for availability planning). The same-day availability rule is neutralised for play-up presentation so players can see and plan opportunities; selection legality (including same-day blocks) is always computed from the true Registered Team at selection time.
